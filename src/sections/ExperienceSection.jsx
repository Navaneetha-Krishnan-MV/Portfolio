...
import { Briefcase, Code, Wrench, Brain, Database } from 'lucide-react';
...
const experiences = [
  {
    ...
    description: [
      'Built and enhanced 10+ production features using React.js, including mobile-responsive UI improvements and resolving high-priority UI bugs in a live product.',
      'Integrated frontend with NestJS backend APIs using Prisma ORM, ensuring reliable data flow and feature stability.',
      'Developed advanced dashboard functionality such as drill-through analytics and real-time LLM token usage wallet updates.'
    ],
    techStack: ['React.js', 'PostgreSQL', 'TypeScript', 'NestJS', 'Prisma ORM', 'Git'],
    icon: Database,
    color: 'cyan'
  },
  {
    ...
    description: [
      'Developed an AI-based assistive system using YOLO for real-time detection of 5+ object categories, improving environmental awareness for visually challenged users.',
      'Integrated real-time text-to-speech feedback, reducing user navigation errors by ~30% during testing. Designed a visually challenged-friendly web interface for intuitive interaction and accessibility.',
    ],
    techStack: ['Django', 'Object Detection', 'Text-to-Speech', 'Artificial Intelligence (AI)'],
    icon: Brain,
    color: 'purple'
  },
  {
...
const ExperienceSection = () => {
  ...
  useEffect(() => {
    highlightedCardRef.current = highlightedCard;
  }, [highlightedCard]);
  ...
  useEffect(() => {
    ...
    cardsRef.current = Array.from(sectionEl.querySelectorAll('[data-experience-id]'));
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        if (timelineRef.current) {
          ...
          setTimelineHeight((prevHeight) => (
            Math.abs(prevHeight - targetHeight) < 0.5 ? prevHeight : targetHeight
          ));
        }
        ...
        if (closestCard !== highlightedCardRef.current) {
          highlightedCardRef.current = closestCard;
          ...
          return (
            ...
            >
              ...
              <div className="absolute left-1/2 transform -translate-x-1/2 z-20">
                <div
                  className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-700 ${isHighlighted
                ...
                >
                  <experience.icon className={`w-7 h-7 transition-colors duration-700 ${isHighlighted ? 'text-white' : colors.text
                    }`} />
                  {isHighlighted && (
                ...
                <>
                  ...
                  <div className="w-1/2 pr-16">
                    <div className="flex justify-end">
                      <div
                        className={`max-w-md transform transition-all duration-700 ${isHighlighted
                            ? 'translate-y-0 scale-[1.02]'
                            : 'translate-y-2 scale-[0.98]'
                          }`}
                      >
                        {/* Animated border */}
                        <div className={`absolute -inset-1 rounded-xl blur transition-all duration-700 ${isHighlighted
                        ...
                        {/* Card */}
                        <div className={`relative backdrop-blur-md rounded-xl p-6 transition-all duration-700 ${isHighlighted
                            ? `bg-black/95 border ${colors.border} ${colors.glow} shadow-xl`
                            : 'bg-black/80 border border-gray-700/40'
                          }`}>
                          {/* Company header */}
                          <div className="flex items-center gap-3 mb-4">
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold transition-all duration-700 ${isHighlighted
                            ...
                            <div>
                              <h3 className={`text-xl font-semibold transition-colors duration-700 ${isHighlighted ? 'text-white' : 'text-gray-300'
                                }`}>
                                {experience.company}
                              </h3>
                              <p className={`text-sm transition-colors duration-700 ${isHighlighted ? colors.text : 'text-gray-500'
                                }`}>
                                {experience.role}
                              </p>
                            </div>
                          ...
                          <div className="mb-4">
                            <ul className="space-y-2">
                              {experience.description.map((desc, idx) => (
                                <li key={idx} className={`text-sm flex items-start gap-2 transition-colors duration-700 ${isHighlighted ? 'text-gray-200' : 'text-gray-400'
                                  }`}>
                                  <span className={`w-1.5 h-1.5 rounded-full mt-2 flex-shrink-0 transition-colors duration-700 ${isHighlighted ? colors.bg.replace('/10', '/70') : 'bg-gray-600/50'
                                    }`} />
                                  {desc}
                                </li>
                              ))}
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {experience.techStack.map((tech, idx) => (
                              <Badge
                                key={idx}
                                variant="outline"
                                className={`text-xs transition-all duration-700 ${isHighlighted
                                    ? `${colors.bg} ${colors.border} border ${colors.text}`
                  ...
                  <div className="w-1/2 pl-16">
                    <div className={`transition-all duration-700 ${isHighlighted ? 'translate-y-0 opacity-100' : 'translate-y-1 opacity-80'
                      }`}>
                      <div className={`inline-block px-4 py-2 backdrop-blur-sm rounded-full transition-all duration-700 ${isHighlighted
                          ? `${colors.bg} ${colors.border} border`
                          : 'bg-gray-800/60 border border-gray-600/40'
                        }`}>
                        <span className={`text-sm font-medium transition-colors duration-700 ${isHighlighted ? colors.text : 'text-gray-400'
                          }`}>
                          {experience.duration}
                        </span>
                      </div>
    ...
    <section ref={sectionRef} className="min-h-screen py-16 px-4 bg-black relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <p className={`text-gray-400 max-w-3xl mx-auto leading-relaxed ${isMobile ? 'text-base' : 'text-lg'
            }`}>
